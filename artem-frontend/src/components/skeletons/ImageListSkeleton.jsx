  ...
  function ImageListSkeleton({noTitle, length=10}) {
    return (
      <Box>
        ...
        {noTitle ? null : <Container>
          <Skeleton animation="wave" variant="text" width={90} sx={{ fontSize: '1.2rem', mb: theme.spacing(3) }} />
        </Container>}
        <ImageListMUI
          cols={3}
          gap={1}
        ...
        >
          {Array.from(Array(length || 10).keys()).map((post, index) => (
            <ImageListItem key={post}>
              ...
              <Skeleton animation="wave" variant="rectangular" width={140} height={140}/>
            </ImageListItem>
          ))}
        </ImageListMUI>
      </Box>
